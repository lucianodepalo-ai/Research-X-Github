"use client";

import { useEffect, useState, useRef } from "react";

interface MonitorEvent {
  id: number;
  source: string;
  level: string;
  message: string;
  detail: string | null;
  created_at: string;
}

interface LiveStats {
  reposToday: number;
  reposTotal: number;
  blogsToday: number;
  twitterToday: number;
  lastGithubLive: string | null;
  lastAnthropic: string | null;
  lastHN: string | null;
  lastDevto: string | null;
}

const SOURCE_META: Record<string, { label: string; emoji: string; color: string }> = {
  github_live:  { label: "GitHub Live",    emoji: "🐙", color: "text-emerald-400" },
  anthropic:    { label: "Anthropic Blog", emoji: "📢", color: "text-blue-400"    },
  hackernews:   { label: "Hacker News",    emoji: "🟡", color: "text-amber-400"   },
  devto:        { label: "Dev.to",         emoji: "📝", color: "text-purple-400"  },
  twitter_rss:  { label: "Twitter RSS",    emoji: "🐦", color: "text-sky-400"     },
  monitor:      { label: "Monitor",        emoji: "⚙️", color: "text-slate-400"   },
};

const LEVEL_COLOR: Record<string, string> = {
  info:    "text-slate-300",
  success: "text-emerald-400",
  warn:    "text-amber-400",
  error:   "text-red-400",
};

function relativeTime(iso: string): string {
  const diff = Math.floor((Date.now() - new Date(iso).getTime()) / 1000);
  if (diff < 60) return `${diff}s`;
  if (diff < 3600) return `${Math.floor(diff / 60)}m`;
  return `${Math.floor(diff / 3600)}h`;
}

function formatTime(iso: string): string {
  return new Date(iso).toLocaleTimeString("es-AR", {
    hour: "2-digit", minute: "2-digit", second: "2-digit",
    timeZone: "America/Argentina/Buenos_Aires",
  });
}

function lastRunBadge(iso: string | null): string {
  if (!iso) return "Sin datos";
  const diff = Math.floor((Date.now() - new Date(iso).getTime()) / 1000);
  if (diff < 300) return "Hace " + diff + "s ✓";
  if (diff < 3600) return "Hace " + Math.floor(diff / 60) + "m";
  return "Hace " + Math.floor(diff / 3600) + "h";
}

export default function LivePage() {
  const [events, setEvents] = useState<MonitorEvent[]>([]);
  const [stats, setStats] = useState<LiveStats | null>(null);
  const [lastUpdate, setLastUpdate] = useState<string>("");
  const [running, setRunning] = useState(false);
  const logRef = useRef<HTMLDivElement>(null);
  const autoScrollRef = useRef(true);

  async function fetchData() {
    try {
      const r = await fetch("/api/live", { cache: "no-store" });
      if (!r.ok) return;
      const data = await r.json();
      setEvents(data.events);
      setStats(data.stats);
      setLastUpdate(new Date().toLocaleTimeString("es-AR", { timeZone: "America/Argentina/Buenos_Aires" }));
      setRunning(true);
    } catch {
      setRunning(false);
    }
  }

  useEffect(() => {
    fetchData();
    const interval = setInterval(fetchData, 8000);
    return () => clearInterval(interval);
  }, []);

  useEffect(() => {
    if (autoScrollRef.current && logRef.current) {
      logRef.current.scrollTop = logRef.current.scrollHeight;
    }
  }, [events]);

  const sources = Object.entries(SOURCE_META);

  return (
    <div className="space-y-4 max-w-5xl">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold mono text-primary">Monitor en Vivo</h1>
          <p className="text-xs text-muted-foreground mt-0.5">
            Auto-refresh cada 8s · Última actualización: {lastUpdate}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <span className={`w-2 h-2 rounded-full ${running ? "bg-emerald-400 animate-pulse" : "bg-red-400"}`} />
          <span className="text-xs text-muted-foreground">{running ? "Corriendo" : "Sin conexión"}</span>
        </div>
      </div>

      {/* Stats grid */}
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard label="Repos hoy" value={stats.reposToday} sub={`${stats.reposTotal} total`} color="text-emerald-400" />
          <StatCard label="Blog posts hoy" value={stats.blogsToday} color="text-blue-400" />
          <StatCard label="Tweets relevantes" value={stats.twitterToday} color="text-sky-400" />
          <StatCard label="GitHub total" value={stats.reposTotal} color="text-primary" />
        </div>
      )}

      {/* Source status */}
      {stats && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
          {[
            { key: "GitHub Live",    last: stats.lastGithubLive, emoji: "🐙" },
            { key: "Anthropic Blog", last: stats.lastAnthropic,  emoji: "📢" },
            { key: "Hacker News",    last: stats.lastHN,         emoji: "🟡" },
            { key: "Dev.to",         last: stats.lastDevto,       emoji: "📝" },
          ].map(({ key, last, emoji }) => (
            <div key={key} className="bg-card border border-border rounded-md px-3 py-2">
              <p className="text-xs text-muted-foreground">{emoji} {key}</p>
              <p className="text-xs mono text-foreground mt-0.5">{lastRunBadge(last)}</p>
            </div>
          ))}
        </div>
      )}

      {/* Event log */}
      <div className="bg-[#050505] border border-border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card">
          <span className="text-xs font-medium text-muted-foreground mono">LOG DEL MONITOR</span>
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer">
            <input
              type="checkbox"
              defaultChecked
              onChange={(e) => { autoScrollRef.current = e.target.checked; }}
              className="rounded"
            />
            Auto-scroll
          </label>
        </div>
        <div
          ref={logRef}
          className="h-[420px] overflow-y-auto p-3 space-y-0.5 font-mono text-xs"
          style={{ scrollbarWidth: "thin" }}
        >
          {events.length === 0 ? (
            <p className="text-muted-foreground text-center py-8">
              Sin eventos aún. El monitor registra actividad aquí en tiempo real.
            </p>
          ) : (
            [...events].reverse().map((ev) => {
              const src = SOURCE_META[ev.source] ?? { label: ev.source, emoji: "•", color: "text-slate-400" };
              const lvlColor = LEVEL_COLOR[ev.level] ?? "text-slate-300";
              return (
                <div key={ev.id} className="flex items-start gap-2 py-0.5 hover:bg-white/[0.02] px-1 rounded">
                  <span className="text-muted-foreground/40 w-16 shrink-0 text-[10px] pt-0.5">
                    {formatTime(ev.created_at)}
                  </span>
                  <span className={`${src.color} w-24 shrink-0 truncate text-[10px] pt-0.5`}>
                    {src.emoji} {src.label}
                  </span>
                  <span className={`${lvlColor} flex-1`}>{ev.message}</span>
                  {ev.detail && (
                    <span className="text-muted-foreground/40 text-[10px] shrink-0 pt-0.5 max-w-[160px] truncate">
                      {ev.detail}
                    </span>
                  )}
                </div>
              );
            })
          )}
        </div>
      </div>
    </div>
  );
}

function StatCard({ label, value, sub, color }: { label: string; value: number; sub?: string; color: string }) {
  return (
    <div className="bg-card border border-border rounded-lg p-3">
      <p className="text-xs text-muted-foreground">{label}</p>
      <p className={`text-2xl font-bold mono ${color} mt-1`}>{value}</p>
      {sub && <p className="text-xs text-muted-foreground/60 mt-0.5">{sub}</p>}
    </div>
  );
}
